...
import static org.marelias.contacts.utils.VCardUtils.getMobileNumber;
import static org.marelias.contacts.utils.VCardUtils.getNameFromVCard;
import static org.marelias.contacts.utils.VCardUtils.isFavorite;
import static org.marelias.contacts.utils.VCardUtils.isPrimaryPhoneNumber;
import static org.marelias.contacts.utils.VCardUtils.markPrimaryPhoneNumberInVCard;
...
public class ContactsDBHelper {
    ...
    static List<org.marelias.contacts.domain.Contact> getAllContactsFromDB() {
        List<PhoneNumber> dbPhoneNumbers = PhoneNumber.listAll(PhoneNumber.class);
        HashMap<Long, org.marelias.contacts.domain.Contact> contactsMap = new HashMap<>();
        org.marelias.contacts.domain.Contact tempContact;
        for (PhoneNumber dbPhoneNumber : dbPhoneNumbers) {
    ...
    static org.marelias.contacts.domain.Contact getContact(long id) {
        if (id == -1)
            return null;
        org.marelias.contacts.orm.Contact contact = ContactsDBHelper.getDBContactWithId(id);
        if (contact == null)
            return null;
        return createNewDomainContact(contact, contact.getAllPhoneNumbers());
    }

    static void togglePrimaryNumber(String mobileNumber, org.marelias.contacts.domain.Contact contact, Context context) {
    ...
    static void updateLastAccessed(long contactId, String callTimeStamp) {
        org.marelias.contacts.orm.Contact contact = ContactsDBHelper.getDBContactWithId(contactId);
